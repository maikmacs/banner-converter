...
if ($_FILES["zip_file_HB"]["name"]) {
	$filename = $_FILES["zip_file_HB"]["name"];
	$source = $_FILES["zip_file_HB"]["tmp_name"];
	$type = $_FILES["zip_file_HB"]["type"];
	...
	$path = preg_replace('/\\.[^.\\s]{3,4}$/', '', $_FILES["zip_file_HB"]["name"]);
	...
	if (move_uploaded_file($source, $target_path)) {
		...
		$folder_name = basename($_FILES["zip_file_HB"]["name"], ".zip");
